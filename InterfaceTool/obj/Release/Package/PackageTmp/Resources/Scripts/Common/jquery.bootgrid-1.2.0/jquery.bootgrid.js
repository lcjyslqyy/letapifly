...
; (function ($, window, undefined) {
    ...
    function getRequest() {
        ...
        if (this.searchData) {
            post = $.extend(post, this.searchData);
        }
    ...
    */
        ...
        if (this.options.ajax) {
            ...
            var settings = {
                ...
                success: function (response) {
                    ...
                    //console.log(that)
                    //console.log(that)
                    //that.pageSelect = true;
                    var page_select = $(that.footer).find(".bg_page_select");
                    page_select.empty();
                    for (var i = 1; i <= that.totalPages; i++) {                     
                        page_select.append('<option value="' + i + '" ' + (that.current == i ? 'selected' : '') + ' >第' + i + '页</option>');
                    }
                    //page_select.off()
                    if (!that.pageSelect) {
                        that.pageSelect = true;
    ...
    Grid.defaults = {
        ...
        labels: {
            all: "100",
            infos: " 第 {{ctx.start}} 到 {{ctx.end}} 行，一共 {{ctx.total}} 行",
            loading: "读取中...",
            noResults: "没有记录",
            refresh: "刷新",
            search: "搜索"
        ...
        templates: {
            ...
            footer: "<div id=\"{{ctx.id}}\" class=\"{{css.footer}}\"><div class=\"row\"><div class=\"col-sm-6\"><p class=\"{{css.pagination}}\"></p></div><div style='margin-top:7px' class=\"col-sm-6 infoBar\"><select style='float:right;margin:0 5px' class=\"bg_page_select\"></select> <p class=\"{{css.infos}}\"></p></div></div></div>",
